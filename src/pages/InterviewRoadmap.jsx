...
const InterviewRoadmap = () => {
  ...
  useEffect(() => {
    const loadProgress = async () => {
      try {
        setIsLoading(true);
        const data = await progressService.getInterviewProgress();
  ...
  const toggleComplete = async (id) => {
    ...
    setCompletedItems((prev) => ({ ...prev, [id]: newCompleted }));

    // Save to API
    try {
      setIsSaving(true);
      await progressService.updateInterviewProgress(id, newCompleted);
    } catch (error) {
